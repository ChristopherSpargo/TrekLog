...
export class GoalsSvc {
  ...
  buildIntervalList = (gdo: GoalDisplayObj) => {
    ...
    let iDate = new Date(
      this.utilsSvc.dateFromSortDateYY(gdo.goal.dateSet)
    ).getTime();
    ...
    do {
      ...
      if (gdo.goal.category === "Cumulative Activity") {
        switch (gdo.goal.testUnits) {
          ...
          case "monthly":
            let im = new Date(
              this.utilsSvc.dateFromSortDateYY(intervalDate)
            ).getMonth();
            iDate += MS_PER_DAY * DAYS_IN_MONTHS[im];
            break;
        }
        int.end = this.utilsSvc.formatSortDate(
          new Date(iDate - MS_PER_DAY).toLocaleDateString(),
          "11:59 PM"
        );
        intervalDate = this.utilsSvc.formatSortDate(
          new Date(iDate).toLocaleDateString(),
          "12:00 AM"
        );
      } else {
